...
単純な上書きだと後勝ちで前のデータが消える。それを防ぐのが `mergeStates`([`src/lib/progress.ts`](../src/lib/progress.ts))。フィールドごとに合わせ方が違う:

| フィールド | マージ規則 |
|---|---|
| 解答履歴 attempts | 両端末の**和集合**(重複除去)。どちらの記録も消えない |
| 復習 review | **エントリ単位のLWW**(各エントリの `u`=更新時刻が新しい方)。卒業は削除ではなく**墓標**(`box:5, due:9999-12-31`)として残す — 削除だと古いスナップショットから復活してしまうため |
| ことば帳 vocab | エントリ単位のLWW(`u`)。バックフィル由来は `u=0` で、実操作に必ず負ける |
| 設定 settings | **キー単位のLWW**(`settings.meta` のキー別時刻)。**削除も時刻を打つ**ので「シャッフルONに戻す」「試験日クリア」が同期で復活しない |
| 午後採点 pm | **設問パーツ単位のLWW**(採点時刻 `t`)。AI講評 `ai` は独立に `ai.t` の新しい方を保持 |
| 実績 achievements | フィールド単位の統合(解除は早い方・進捗は大きい方) |
| 未知のフィールド | **温存**(旧クライアントが新フィールドを落とさないための防御) |

タイムスタンプの無い旧形式のデータは「全体の更新時刻が新しい端末を優先、片方にしか無いものは残す」の従来規則で扱う(後方互換)。
さらに同期の適用後には、解答履歴のリプレイで「本来は卒業済みなのに生き返っている」復習エントリを墓標化する修復(`repairReviewGraduations`)が走り、過去の欠陥で汚染されたデータも自然に治る。
