---
tags: [設計, 実績]
aliases: [実績システム設計書, アチーブメント]
---

# 応用情報学習アプリ 実績(アチーブメント)システム 最終設計書

> [!info] [[ホーム]] › 実績の詳細設計。データの土台は [[データモデル]]。
...
## 10. 実績カタログ(長期継続版・§4を拡張)

> 追記の目的: 応用情報の学習は数ヶ月に及ぶため、**期間が長くても実績が尽きず、ずっと獲得し続けられる**よう大幅に拡張する。設計原則は据え置き(真実源=`attempts`、純関数導出、追加保存は `achievements` メタのみ)。ここで挙げる条件はすべて現データ(`attempts` + `pm` + `settings.examDate`)から算出可能。

### 10.1 長期継続のための3つの仕掛け

1. **階段を長くする**: 序盤は密(すぐ届く)、後半は間隔を広げる(何ヶ月分もの目標)。累計は最大12,000問、連続は150日まで用意。
2. **「延べ学習日数」を streak とは別軸で用意**: 1日空くと0に戻る `streak` と違い、**休んでも減らない**日数カウント。長期戦で必ず積み上がる。
3. **周期(デイリー/ウィークリー/マンスリー)実績**: 「目標達成した日/週/月の累計回数」。何ヶ月続けても**新しいティアが出続ける**継続エンジン。

ティア凡例: **Br**ブロンズ / **Si**シルバー / **Go**ゴールド / **Pt**プラチナ / **Di**ダイヤモンド。判定信号は§4.1の略号。

### 10.2 量(Volume)— 累計とユニークの2軸(ユーザ最重点)

| カテゴリ | 名称 | 条件 | ティア |
|---|---|---|---|
| 累計 `TOTAL` | 千里の一歩 / 歩みを止めず | 100 / 300 | Br / Br |
| 累計 | 反復の徒 / 千本ノック | 500 / 1,000 | Si / Si |
| 累計 | 二千の壁 / 研鑽の三千 | 2,000 / 3,000 | Go / Go |
| 累計 | 問題の海 / 八千の探求 | 5,000 / 8,000 | Pt / Pt |
| 累計 | 一万二千の頂 | 12,000 | Di |
| ユニーク `UNIQ` | 初見ハンター / 開拓者 | 50 / 150 | Br / Si |

> 最上位 `12,000` は単一問題ループで埋まらないよう `TOTAL>=12000 かつ UNIQ>=400` のAND(§9-3推奨)。

### 10.3 リベンジ(Revenge)— 誤答→正答(ユーザ最重点・独自の核)

| 軸 | 名称 | 条件 | ティア |
|---|---|---|---|
| 延べ `REVENGE_B` | 初リベンジ | 1 | Br |
| 延べ | 雪辱の一撃 / 雪辱を重ねて | 10 / 30 | Br / Si |
| 延べ | 七転八起 / 不屈の闘志 | 50 / 100 | Si / Go |
| 延べ | 雪辱百戦 / 雪辱の覇者 | 200 / 300 | Go / Pt |
| 延べ | リベンジ王 / 不撓不屈 | 500 / 800 | Pt / Di |
| 克服種類 `REVENGE_C` | 弱点撃破 / 弱点克服 | 10 / 30 | Br / Si |
| 克服種類 | 弱点殲滅 / 弱点マスター | 50 / 100 | Go / Pt |
| 克服種類 | 完全克服 | 200 | Di |

### 10.4 継続(Streak)— 連続と「延べ日数」の2軸

| 軸 | 名称 | 条件 | ティア |
|---|---|---|---|
| 最長連続 | 三日坊主返上 / 一週間皆勤 | 3 / 7 日連続 | Br / Br |
| 最長連続 | 二週間走破 / 継続は力 | 14 / 30 日連続 | Si / Go |
| 最長連続 | 五十日の精進 / 百日修行 | 50 / 100 日連続 | Pt / Di |
| 最長連続 | 不動の継続 | 150 日連続 | Di |
| **延べ学習日数**(休んでも減らない) | 学びの十日 / 三十日生 | 10 / 30 日 | Br / Si |
| 延べ学習日数 | 還暦学習 / 百日到達 | 60 / 100 日 | Go / Pt |
| 延べ学習日数 | 二百日の軌跡 | 200 日 | Di |

### 10.5 網羅(Coverage)= `UNIQ / 640`

| 名称 | 条件 | ティア |
|---|---|---|
| 序盤突破 / 折り返し地点 | 25% / 50% | Br / Si |
| 山を越えて / 大詰め | 75% / 90% | Go / Pt |
| 全問踏破 | 95%(§9-2で100%も選択可) | Di |

### 10.6 分野マスタリー(Mastery)

| 名称 | 条件 | ティア |
|---|---|---|
| 分野制覇 | いずれか1中分類を完答(`MID_DONE`) | Si |
| 三分野の達人 | 3中分類を完答 | Go |
| テクノロジ制覇 / マネジメント制覇 / ストラテジ制覇 | 各大分類の全中分類を完答 | Go / Go / Go |
| 全分野マスター | 全17中分類を完答(生涯目標) | Di |

> 分母は `countByMiddle()`(計算問題込み)。計算問題を避け続けると届かない旨をUIヒントに明記(§4.4-2)。

### 10.7 周期(Recurring)— 長期継続の主エンジン(累計達成回数=単調)

| 周期 | 名称 | 条件(達成の定義→累計) | ティア |
|---|---|---|---|
| デイリー | 精進の芽 / 精進の習慣 | 「1日20問以上」を累計 5 / 15 日 | Br / Si |
| デイリー | 精進の達人 / 精進の鬼 | 同 30 / 100 日 | Go / Di |
| ウィークリー | 週間皆勤 ×N | 「週100問以上」を累計 4 / 12 / 26 週 | Si / Go / Pt |
| マンスリー | ひと月完走 / 三ヶ月完走 / 半年完走 | 「月20日以上学習」を累計 1 / 3 / 6 ヶ月 | Si / Go / Di |

> デイリー/ウィークリー/マンスリーの閾値(20問/100問/20日)は設定で調整可能にする案(§9追加選択肢)。

### 10.8 挑戦・特殊(Challenge)

| 名称 | 条件 | 信号 | ティア |
|---|---|---|---|
| 連勝街道 / 連勝の達人 | 連続正解 10 / 30 | `COMBO` | Si / Go |
| 猛勉強 / 一日の鬼 | 1日で 50 / 100 問 | 日次最大 | Si / Go |
| 朝活 / 夜更かし勉強 | 朝5–8時 / 深夜0–4時 に解答 | `t`の時刻 | Br / Br |
| 週末戦士 | 土日に学習 | `t`の曜日 | Br |
| おかえりなさい | 7日以上空けて学習を再開 | 日付の間隔 | Br |
| 計算も恐れず / 計算マスター | 計算問題を 50 / 200 問正解 | `isCalcQuestion`&`ok` | Si / Go |
| 三刀流 | practice・review・mock すべてで解答 | `mode`の集合 | Si |

### 10.9 午後(PM)・初回・試験(Milestone)

| 名称 | 条件 | ティア |
|---|---|---|
| はじめの一問 / 初正解 | `TOTAL>=1` / 初めて正解 | Br / Br |
| 午後デビュー / 完答者 | 午後を1問採点 / 1問完答 | Br / Si |
| 本気の宣言 | 試験日(`settings.examDate`)を設定 | Br |
| ラストスパート | 試験7日前以内に学習 | Si |
| 走り抜けた | 試験前日まで学習を継続 | Go |

> 午後は現データが1問/8パーツのため下位のみ。PM問題が増えたらしきい値を自動クランプで伸ばす(§4.4-1)。

**合計 約75実績**(§4のMVP17は本カタログの下位ティアに含まれる)。MVPで17→初期公開、以降ティアを段階解放。周期(10.7)と延べ日数(10.4後半)が「何ヶ月やっても実績が出続ける」を担保する。
...
### 11.1 コンセプト
**円形メダル × ティア5色 × カテゴリ別グリフ**。すべて**SVGで動的生成**(画像アセット無し=リポジトリ肥大なし・無限に拡大縮小可・`Icons.tsx`/`ContributionGraph`/`AccuracyTrend` と同じ流儀)。1つの `<Badge tier glyph state progress/>` コンポーネントで全実績を描画。

### 11.2 構成要素(viewBox 100×100)
1. **外リム**: ティア色の縦グラデ(`linearGradient`)の円 r=47。メダルの縁。
2. **内ディスク**: 左上を光源にした放射グラデ(`radialGradient` cx=0.38 cy=0.32)の円 r=40。立体感。
3. **ハイライト**: 上部に白い楕円 opacity 0.2(金属の艶)。
4. **グリフ**: 中央に白(opacity 0.97)のカテゴリ記号。
5. (進行中のみ)**進捗リング**: r=45 に薄いトラック+アクセント青の弧(`stroke-dasharray` で `progress` 分だけ、-90°回転で頂点始点)。
6. (未解除のみ)**グレースケール+南京錠**: `filter:grayscale(1)` + opacity 0.5 + 錠前アイコン。名称は「?????」。

### 11.3 ティア色(メダルの素材感。ライト/ダーク両方で映えるよう固定色)
| ティア | 明→暗グラデ | 意味 |
|---|---|---|
| ブロンズ | `#F0B87E → #A9682F` | 銅 |
| シルバー | `#EEF3F8 → #9BA7B4` | 銀 |
| ゴールド | `#FBE08A → #D09A1E` | 金 |
| プラチナ | `#EAF7F4 → #93C3CF` | 白金(青みの白) |
| ダイヤモンド | `#9FEAF2 → #8FB4F2 → #B58CF0`(3色) | 虹色の輝き |

> バッジのティア色だけは例外的に固定色(金メダルはダークでも金)。周囲のカード/文字は従来どおり `var(--*)` でテーマ追従。

### 11.4 カテゴリ別グリフ(白のSVGパス)
量=積み上げた3本のバー / リベンジ=ループ矢印+チェック(やり直して正解) / 継続=炎 / 網羅=3×3のドット / マスタリー=星 / 周期=カレンダー+チェック / 挑戦=稲妻 / 午後=ペン / 初回=芽。

### 11.5 状態と表示
- **解除済み**: フルカラー。分析タブのグリッドでは `background:var(--accent-bg)` の枠で強調。
- **進行中**: 進捗リング+「62%」等。`progress=Math.max(保存値,現在値)`(後退で萎えさせない)。
- **未解除**: グレー+錠前+「あと◯問」ヒント(`.muted.small`)。ネタバレ防止で条件は概要のみ。
- サイズ: グリッドで56–84px、解除トーストで64px、ホームサマリで40px。同一SVGを拡大縮小で流用。

### 11.6 実装
`src/components/Badge.tsx` を新規作成(props: `tier`, `glyph`, `state`, `progress`)。`ACHIEVEMENTS` 配列の各定義に `tier` と `glyph`(カテゴリ)を持たせ、`AchievementGrid`/`AchievementToast`/ホームサマリが共通で使う。グリフ追加は `Icons.tsx` の `svgProps()` 作法に準拠。
